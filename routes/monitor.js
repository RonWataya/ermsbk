...
// ---------------- FETCH CANDIDATES ----------------
router.get('/candidates/:pollingCenterId', async (req, res) => {
...
router.post('/submit-results',
    ...
    async (req, res) => {
        ...
        const { monitorId, pollingCenterId, electionType, totalVotesCast, invalidVotes, unusedBallots, votes, flag } = req.body; // <-- Add 'flag' here
        ...
        try {
            ...
            // --- NEW: Conditionally update the flag column ---
            if (flag == 1) { // Use `==` for loose comparison since it comes as a string
                await connection.execute(
                    'UPDATE submission_sessions SET flag = 1 WHERE session_id = ?',
                    [sessionId]
                );
            }
...
// ---------------- SUBMISSION HISTORY ----------------
router.get('/submissions/:monitorId', async (req, res) => {
